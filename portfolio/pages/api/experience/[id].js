...
async function handler(req, res) {
  ...
  if (req.method === "PUT") {
    return requireAdmin(async (req, res) => {
      ...
      await db.update(schema.experience).set(parsed.data).where(eq(schema.experience.id, id));
      const [updated] = await db.select().from(schema.experience).where(eq(schema.experience.id, id));
      if (!updated) {
